...
REDIS_KEY_HIST = "mf_weekly_history_v1"

# VIX Term Trigger — external tactical risk indicator (63-day Z-scores)
# We fetch this from the risk-indicator dashboard's public API once per refresh
# and display it in the header pill. Replaces the older 252-day RISK ON/OFF.
VIX_TRIGGER_API_URL   = "https://risk-indicator-v4.vercel.app/api/vix-term-indicator"
VIX_TRIGGER_LINK_URL  = "https://risk-indicator-v4.vercel.app/"
...
cache = {
    ...
    "benchmarks": {},
    "weekly_history": {},   # { "VFIAX": [{date, zscore}, ...], "SPY": [...] }
    "history_status":  "unknown",  # unknown | seeding | ready | error
    # VIX Term Trigger snapshot pulled from the risk-indicator API on each refresh
    "vix_trigger": {
        "signal":    "—",      # "Bullish" | "Neutral" | "Bearish" | "—"
        "composite": None,      # numeric Z-score, e.g. 0.10
        "color":     "#94a3b8", # hex from API, greyed while loading
        "timestamp": "—",       # UTC time string from API
    },
}
...
def save_to_redis():
    ...
    payload = {
        ...
        "vix_trigger":  cache.get("vix_trigger", {}),
    }
...
def load_from_redis():
    ...
    # Preserve default fields if the API hasn't ever returned yet
    saved_vt = payload.get("vix_trigger")
    if saved_vt:
        cache["vix_trigger"] = saved_vt
    ...
    # Weekly history has its own key
...
def zscore_1yr_ending(closes, end_date):
    ...
    Uses the same math as zscore_1yr, but for a specific historical date
    instead of the most recent close. Needed for weekly trail snapshots.

    Returns None if insufficient data or no price on/before end_date.
    """
    if not isinstance(end_date, pd.Timestamp):
        end_date = pd.Timestamp(end_date)
    # Find the closes on or before end_date
...
def fridays_back(n):
    ...
    Weekday: Monday=0..Friday=4..Sunday=6. We find the most recent Friday that is
    strictly before today, then step back by weeks.
    """
    today = pd.Timestamp(date.today())
    days_since_fri = (today.weekday() - 4) % 7
    if days_since_fri == 0:
        # If today is Friday, use last week's Friday to avoid mixing "today" with the trail
        days_since_fri = 7
...
def fetch_vix_trigger():
    """Pull the current VIX Term Trigger reading from the risk-indicator API.

    Returns a dict with signal, composite Z-score, color, and timestamp.
    On error, returns a graceful placeholder so the dashboard still renders.
    Called once per full refresh; result is cached in Redis with everything else.
    """
    try:
        r = requests.get(VIX_TRIGGER_API_URL, timeout=10)
        if r.status_code != 200:
            print(f"  VIX Trigger fetch failed: HTTP {r.status_code}")
            return None
        data = r.json()
        return {
            "signal":    data.get("signal", "—"),
            "composite": data.get("composite"),
            "color":     data.get("signal_color", "#94a3b8"),
            "timestamp": data.get("timestamp", "—"),
        }
    except Exception as e:
        print(f"  VIX Trigger error: {e}")
        return None
...
def run_update():
    with _lock:
        cache["phase"]  = 1
        cache["error"]  = None
        # If there's no prior history, we're doing a fresh seed
    ...
    try:
        ...
        for bench in BENCHMARKS:
            bsym = bench["symbol"]
            try:
                # Need 1yr + 13 weeks lookback for weekly Z-score history; use 2yr for buffer
                ...
                # Weekly history for benchmarks (same as funds)
                bench_history = []
                for fri in fridays_back(13):
        ...
        # VIX (VIXY/VXX) and VIX Term Trigger (external API)
        with _lock:
            cache["progress"] = "Fetching VIX signal..."
        sig, v9, vi = fetch_vix()

        with _lock:
            cache["progress"] = "Fetching VIX Term Trigger..."
        vt = fetch_vix_trigger()
        ...
        with _lock:
            ...
            cache["vix_value"]    = vi
            if vt is not None:
                cache["vix_trigger"] = vt
            # else: keep prior value (better than showing dashes if API blips)
            cache["phase"]        = 4
        ...
        print(f"Done — {len(cache['data'])} funds loaded, "
              f"{len(cache.get('weekly_history', {}))} history series.")
...
def _ensure_started():
    ...
    if not _started:
        ...
        if restored and cache["phase"] == 4:
            # Safety check: if we have fund data but no weekly history,
            # trigger a refresh to seed it (only happens once, first deploy after
            # this feature was added).
            if not cache.get("weekly_history"):
                print("  Full cache from Redis but no weekly history — "
                      "triggering refresh to seed history.")
                trigger_update()
            else:
...
def index():
    ...
    return render_template("index.html",
        ...
        error=snap["error"],
        vix_trigger=snap.get("vix_trigger", {}),
        vix_trigger_url=VIX_TRIGGER_LINK_URL)
